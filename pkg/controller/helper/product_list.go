...
func ProductList(ns string, cl client.Client, providerAccount *helper.ProviderAccount, logger logr.Logger) ([]capabilitiesv1beta1.Product, error) {
	...
	for idx := range productList.Items {
		...
		productProviderAccount, err := helper.LookupProviderAccount(cl, ns, productList.Items[idx].Spec.ProviderAccountRef, logger)
		if err != nil {
