...
func BackendList(ns string, cl client.Client, providerAccount *helper.ProviderAccount, logger logr.Logger) ([]capabilitiesv1beta1.Backend, error) {
	...
	for idx := range backendList.Items {
		...
		backendProviderAccount, err := helper.LookupProviderAccount(cl, ns, backendList.Items[idx].Spec.ProviderAccountRef, logger)
		if err != nil {
			return nil, fmt.Errorf("BackendList: %w", err)
		}
